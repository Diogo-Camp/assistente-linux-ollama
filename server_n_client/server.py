...
try:
    ...
    app.logger.critical(f"Falha ao inicializar Ollama: {str(e)}")
    # Opcional: Desativa rotas que dependem do Ollama
...
def chat():
    if not app.config.get('OLLAMA_ENABLED', True):
        return jsonify({"error": "Serviço Ollama indisponível"}), 503
    try:
